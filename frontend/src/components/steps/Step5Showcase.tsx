...
const getDateString = (row: LooseRecord): string | null => {
  const candidate = row.week_ending ?? row.date ?? row.index ?? row.period;
  return typeof candidate === "string" && candidate.trim() ? candidate : null;
};
const getPrimaryNumericValue = (row: LooseRecord): number | null => {
...
const getStringArray = (value: unknown): string[] =>
  Array.isArray(value)
    ...
    : [];
const prettifyDriverLabel = (key: string) =>
  key.replace(/[_-]+/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
const shouldUseBarForDriver = (rows: Array<Record<string, unknown>>, key: string) => {
  ...
  const renderGraph = (b: GraphBlock) => {
    ...
    if (b.assetId === "driver-series") {
      ...
      if (rows.length === 0 || driverSeriesKeys.length === 0) {
        return <div className="text-xs text-slate-400">No driver data available for this chart.</div>;
      }
      return (
                ...
                <ChartWrap height={260}>
                  {isBar ? (
                    ...
                    <LineChart data={rows}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                      <XAxis {...sharedXAxisProps} />
                      <YAxis domain={domain} tickFormatter={(v) => formatDecimal(Number(v), 2)} tick={{ fontSize: 11 }} />
                      <Tooltip labelFormatter={(v) => fmtLongDate(Number(v))} formatter={tooltipValue} />
                      <Legend />
